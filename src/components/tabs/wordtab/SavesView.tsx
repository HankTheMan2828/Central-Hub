...
export function SavesView({
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-w-[400px] border border-[var(--ch-border)] bg-[var(--ch-bg-base)] rounded-sm overflow-hidden">
      <header className="px-4 py-2 border-b border-[var(--ch-border-subtle)] flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 max-w-[320px] ml-2 relative">
        ...
        <button
          ...
          onClick={onNew}
          title="New document"
          className="ml-auto flex items-center gap-1.5 px-3 py-1.5 border border-[#FFB347]/40 rounded-sm text-[10px] uppercase tracking-widest font-mono text-[var(--ch-accent)] hover:bg-[var(--ch-accent-10)] transition-colors"
        >
        ...
        {docs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <button
              type="button"
              onClick={onNew}
              className="flex items-center gap-1.5 px-4 py-2 border border-[#FFB347]/40 rounded-sm text-[10px] uppercase tracking-widest font-mono text-[var(--ch-accent)] hover:bg-[var(--ch-accent-10)] transition-colors"
            >
              <FilePlus2 className="w-3.5 h-3.5" />
            ...
            {filtered.map((doc) => {
              ...
              return (
                <div
                  ...
                  className={`group relative aspect-square flex flex-col p-3 rounded-sm border cursor-pointer transition-colors text-left focus:outline-none ${
                    isActive
                ...
                >
                  ...
                  <div className="mt-2 flex-1 overflow-hidden text-[10px] text-[var(--ch-text-faint)] leading-relaxed line-clamp-6">
                    {doc.snippet || (
