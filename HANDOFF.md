...
## 2026-05-07 — Plain Chat tools rework (issue #6, item 4)

Plain Chat was running with the same fully-toolled PI session as Coding Agent — so the model could (and did) call `bash`, `read`, etc., and the screenshot in #6 showed `Used bash` / `Used read` lines under THINKING blocks. Fixed by giving Plain Chat its own session type that disables built-in coding tools and registers two custom tools instead.

- New `frontend/main/ipc-plain-session.js` registers `pi:plain-session-create`. Mirrors the `pi:word-session-create` pattern: `createSession({ noTools: 'builtin', customTools: [webSearch, weather] })`, subscribes events to `pi:event`, stores in `piSessions`. Wired into `main.js` next to the other `require('./main/ipc-*')` lines.
- `web_search` tool: thin wrapper over the Brave Search REST API. Reads `BRAVE_API_KEY` from `process.env` (set by `brave:set-key` in `ipc-search.js`). When unset, the tool returns a "not configured" message instead of throwing — so the model can degrade gracefully.
- `weather` tool: Open-Meteo (free, no API key). Geocodes the location string, then fetches current conditions plus a 3-day forecast. Defaults to metric, supports `units: "imperial"`.
- `usePiChat` `sessionType` widened from `"chat" | "word"` to `"chat" | "word" | "plain"`. The three previous inline `sessionType === "word" ? ... : ...` channel selectors collapsed into a `sessionCreateChannelFor(type)` helper. `ChatPanel.tsx` now calls `usePiChat({ sessionType: "plain" })`.
- No render changes — `ChatPanel.tsx:838` was already rendering tool calls as a tiny grey italic line, which is what #6 asked for. The fix was that bash/read shouldn't have been running in Plain mode at all.
- Verified: `npx tsc --noEmit` passes.
- Out of scope (still open on #6): persistent chats in Coding Agent, animation perf (unspecific — needs a repro).

Follow-up same day, same branch:

- **Single assistant bubble per agent turn.** Plain Chat was painting a fresh `PI` bubble (with its own THINKING block) for every LLM turn inside one user request — so a tool-using turn produced 3+ bubbles with three separate THINKING headers. Moved bubble creation in `usePiChat.ts` from `message_start` to `agent_start`, made `message_start` a no-op for assistants, and shifted `isStreaming=false` finalization from `message_end` to `agent_end`. The find-last-streaming-assistant lookups in the delta path keep working unchanged, so all thinking deltas across all sub-turns now accumulate into the one bubble. Tool indicators (`Used weather`, etc.) still render as tiny grey lines in the message list, just below the consolidated bubble instead of between split bubbles. This affects every panel that uses `usePiChat` (Plain, Coding Agent, Docs Area), but Henry's complaint was Plain-Chat-specific.
- **Stop the weather/web_search loop.** Plain Chat's model called `weather` ~10 times in a row before answering. Tightened both tools' `promptGuidelines` to "AT MOST ONCE per user question" plus a sentence in the *tool result text itself* that tells the model the data it just got is the answer and not to call again. Putting the instruction in the tool result rather than only in the prompt because the result is the freshest context the model sees before deciding whether to loop.
- **Coding Agent chat titles match Plain Chat.** Coding Agent was setting its tab title from the first 54 chars of the user's message ([CodingAgentPanel.tsx:988](frontend/src/components/CodingAgentPanel.tsx:988)). Added a new effect that, after the first user → assistant exchange completes, fires `pi:generate-title` (the existing OpenRouter qwen3-8b call, ~20 output tokens) and replaces the truncation with the LLM-generated 2-5 word title — both on the active tab and on the persisted chat record. Tracked per chat record id via `titledChatIdsRef` so it only fires once per chat in a session. The existing pi:generate-title channel was already in place; no main-process changes needed.
- Verified: `npx tsc --noEmit` passes.
